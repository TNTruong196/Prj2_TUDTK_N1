...
class TestMatrixHelper(unittest.TestCase):
    ...
    def test_mat_sub(self):
        ...
        self.assertEqual(mat_sub([[10, 20]], [[3, 5]]), [[7, 15]])

    def test_mat_inverse(self):
        # Test 1: Ma tran 2x2
        A = [[4, 7], [2, 6]]
        invA = mat_inverse(A)
        self.assertAlmostEqual(invA[0][0], 0.6, places=9)
        self.assertAlmostEqual(invA[1][1], 0.4, places=9)
        ...
        for i in range(3):
            for j in range(3):
                self.assertAlmostEqual(identity[i][j], 1.0 if i == j else 0.0, places=9)
    ...
    def test_mat_identity(self):
        # Test 1: Don vi 2x2
        self.assertEqual(mat_identity(2), [[1, 0], [0, 1]])
        # Test 2: Don vi 3x3
        self.assertEqual(mat_identity(3), [[1, 0, 0], [0, 1, 0], [0, 0, 1]])
    ...
    def test_mat_add(self):
        ...
        self.assertEqual(mat_add([[2]], [[3]]), [[5]])

    def test_mat_scalar_mul(self):
        # Test 1: Nhan ma tran voi scalar duong
        A = [[1, 2], [3, 4]]
        self.assertEqual(mat_scalar_mul(A, 2), [[2, 4], [6, 8]])
        # Test 2: Nhan ma tran voi scalar am
        self.assertEqual(mat_scalar_mul(A, -1), [[-1, -2], [-3, -4]])
